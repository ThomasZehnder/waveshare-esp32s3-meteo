...
void ui_Service_Screen_init(void)
{
    ...
    //show MQTT status
    mqtt_status_label = lv_label_create(UI_Screens.Service_Screen);   
    lv_label_set_text_fmt(mqtt_status_label, "MQTT Connected: %s", Asset.mqttConnected ? "Yes" : "No");
    lv_obj_align(mqtt_status_label, LV_ALIGN_TOP_LEFT, 0, 60);    
